...
class TodoService:
    ...
    def __init__(
        self, repository: NotionTodoRepository, logger: Optional[Callable] = None
    ):
    ...
    async def add_todo(
        ...
        try:
            ...
            todo = Todo(
                ...
                project_names=(
                    [project_name]
                    if project_name and project_id != "UNKNOWN_PROJECT"
                    else []
                ),
            )
    ...
    async def get_daily_top_tasks(self) -> str:
        ...
        try:
            ...
            daily_top_tasks = [
                todo
                for todo in todos
                if todo.priority == TodoPriority.DAILY_TOP_TASK.value
            ]
    ...
    async def _delete_completed_todos(self) -> None:
        try:
            ...
            delete_tasks = [
                self.repository.delete_todo(todo_id) for todo_id in completed_todo_ids
            ]
