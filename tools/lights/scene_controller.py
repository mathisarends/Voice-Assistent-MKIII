...
class SceneController:
    ...
    async def get_scene_names(self) -> list[str]:
        ...
        return [scene_data.get('name') for scene_id, scene_data in scenes.items() 
    ...
    async def activate_next_scene(self) -> list:
        ...
        """
        active_group, next_scene_id, next_scene_name = await self._get_adjacent_scene(direction=1)
    ...
    async def activate_previous_scene(self) -> list:
        ...
        Returns:
            list: Antwort der Bridge API
        """
        active_group, prev_scene_id, prev_scene_name = await self._get_adjacent_scene(direction=-1)
