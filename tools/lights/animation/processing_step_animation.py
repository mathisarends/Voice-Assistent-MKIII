...
class LightAnimation(ABC, LoggingMixin):
    ...
    async def _run_continuous(
        self, light_ids: List[str], interval: float, **kwargs
    ) -> None:
...
class RotateColorsAnimation(LightAnimation):
    ...
    async def _execute_once(
        ...
        if dim_percentage > 0:
            ...
            for light_id, state in color_states.items():
                if "bri" in state:
                    ...
                    dim_tasks.append(
                        self.controller.set_light_state(light_id, dim_state)
                    )
        ...
        self.logger.info(
            f"Farben zwischen {light_ids} rotiert"
            + (
                f" mit {dim_percentage*100}% Helligkeitsreduzierung"
                if dim_percentage > 0
                else ""
            )
        )
...
class ErrorFlashAnimation(LightAnimation):
    ...
    async def _execute_once(
        self, light_ids: List[str], transition_time: int = 2, hold_time: float = 0.05
    ) -> None:
        """
        ...
        try:
            ...
            for light_id, original_state in original_states.items():
                ...
                restore_tasks.append(
                    self.controller.set_light_state(light_id, restore_state)
                )
    ...
    async def execute(
        ...
        **kwargs,
        ...
        if continuous:
            ...
            self._running_task = asyncio.create_task(
                self._execute_continuous(
                    light_ids, flash_interval=flash_interval, **kwargs
                )
            )
        else:
            # Führe die Animation einmal aus
            ...
            while not self._stop_event.is_set():
                ...
                try:
                    await asyncio.wait_for(
                        self._stop_event.wait(), timeout=flash_interval
                    )
                except asyncio.TimeoutError:
                    pass
        finally:
...
async def main() -> None:
    ...
    await rotate_anim.execute(["5", "6", "7"], transition_time=15, dim_percentage=0.0)
