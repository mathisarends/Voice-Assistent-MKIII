...
class SceneBasedSunriseController:
    ...
    async def get_scene_light_states(self, scene_name: str) -> Tuple[str, Dict[str, Dict[str, Any]]]:
        """
    ...
    async def start_scene_based_sunrise(self, 
        ...
        try:
            ...
            self.running_sunrise = asyncio.create_task(
                self._run_scene_based_sunrise(target_light_ids, target_states, 
                                             duration, start_brightness_percent, 
                                             original_states)
            )
    ...
    async def _run_scene_based_sunrise(self, 
                                      light_ids: List[str],
        ...
        """
        try:
            print(f"Starte Szenen-basierten Sonnenaufgang für {len(light_ids)} Lichter über {duration} Sekunden...")
            ...
            for step in range(1, steps + 1):
                ...
                for light_id in light_ids:
                    if light_id in target_states:
                        ...
                        current_brightness = int(initial_brightness + (target_brightness - initial_brightness) * progress)
