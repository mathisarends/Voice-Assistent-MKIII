...
class AudioCapture(LoggingMixin):
    ...
    def __init__(self, chunk_size: int = 1024, sample_rate: int = 44100, 
                 history_size: int = 5, smoothing_factor: float = 0.3):
        ...
        # Zur Normalisierung
        self.energy_max = {band: 0.1 for band in self.freq_bins}  # Anfangswert um Division durch Null zu vermeiden
    ...
    def _analyze_audio(self, audio_data: bytes) -> None:
        ...
        try:
            ...
            current_time = time.time()  # Verwende time.time() statt asyncio.get_event_loop().time()
            ...
            for band, (low_freq, high_freq) in self.freq_bins.items():
                ...
                if len(self.history[band]) > 0:
                    ...
                    self.energy_levels[band] = (self.smoothing_factor * band_energy + 
                                              (1 - self.smoothing_factor) * self.energy_levels[band])
                
                # Beat-Erkennung (einfacher Schwellenwert-basierter Ansatz)
                avg_history = sum(self.history[band]) / len(self.history[band]) if self.history[band] else 0
                if (band_energy > self.beat_threshold * avg_history and 
                    current_time - self.last_beat_time[band] > 0.1):  # Mindestens 100ms zwischen Beats
    ...
    def get_energy_normalized(self, band: str) -> float:
        ...
        value = (self.energy_levels[band] - self.energy_min[band]) / (self.energy_max[band] - self.energy_min[band])
...
class AudioReactiveAnimation(LoggingMixin):
    ...
    @abstractmethod
    async def _process_audio_frame(self, light_ids: List[str], audio_levels: Dict[str, float], 
                                 beats: Dict[str, bool], **kwargs) -> None:
        """
    ...
    async def _animation_loop(self, light_ids: List[str], **kwargs) -> None:
        ...
        try:
            while not self._stop_event.is_set():
                ...
                await self._process_audio_frame(light_ids, audio_levels, beats, **kwargs)
...
class BeatPulseAnimation(AudioReactiveAnimation):
    ...
    async def _process_audio_frame(self, light_ids: List[str], audio_levels: Dict[str, float], 
                                 beats: Dict[str, bool], color_mode: str = 'bass',
                                 brightness_mode: str = 'energy', intensity: float = 1.0, 
                                 color_shift: bool = True, **kwargs) -> None:
        """
        ...
        brightness_mode = brightness_mode if brightness_mode in ['energy', 'beat'] else 'energy'
        ...
        for light_id in light_ids:
            ...
            if brightness_mode == 'energy':
                ...
                brightness_value = (audio_levels['bass'] * 0.5 + 
                                   audio_levels['mid'] * 0.3 + 
                                   audio_levels['high'] * 0.2)
                brightness = int(70 + 185 * brightness_value * intensity)  # 70-255 Range
            else:  # 'beat' Modus
                ...
                if beats[color_mode]:
                    ...
                    brightness = int(70 + 100 * energy * intensity)  # Gedämpfte Helligkeit ohne Beat
