...
class SpotifyPlayer:
    ...
    def search_track(self, query):
        ...
        for track in tracks:
            ...
            if any(
                any(keyword in artist for keyword in artist_keywords)
                for artist in track_artists
            ):
                print(
                    f"🎵 Gefunden (Künstler-Match): {track['name']} - {track['artists'][0]['name']}"
                )
                return track["uri"]

        track = tracks[0]
        print(
            f"🎵 Gefunden (Erstes Ergebnis): {track['name']} - {track['artists'][0]['name']}"
        )
        return track["uri"]
        ...
        if playlist_uri:
            try:
                self.client.start_playback(
                    device_id=self.device_id, context_uri=playlist_uri
                )
    ...
    def start_concentration_phase(self, playlist_type=None):
        ...
        if not self.device_id:
            print(
                "❌ Gerät MATHISPC nicht verfügbar - Konzentrationsphase kann nicht gestartet werden"
            )
            return None
        ...
        try:
            ...
            if analysis:
                ...
                for i, section in enumerate(
                    analysis["sections"][:5]
                ):  # Zeige nur die ersten 5 Sektionen
                    print(
                        f"   Sektion {i+1}: Start {round(section['start'], 2)}s, Länge {round(section['duration'], 2)}s, "
                        + f"Tempo {round(section['tempo'])} BPM, Lautstärke {round(section['loudness'])} dB"
                    )

                if len(analysis["sections"]) > 5:
                    print(
                        f"   ... und {len(analysis['sections']) - 5} weitere Sektionen"
                    )
    ...
    def get_frequency_spectrum(self, track_uri=None):
        ...
        for i, (desc, value) in enumerate(zip(timbre_desc, avg_timbre)):
            normalized = (
                value + 100
            ) / 200  # Normalisierung auf 0-1 (Werte sind typischerweise zwischen -100 und +100)
