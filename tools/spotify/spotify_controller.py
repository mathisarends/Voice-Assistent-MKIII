...
from tools.spotify.spotify_commands import (
    GetActiveDevicesCommand,
    GetCurrentTrackCommand,
    NextTrackCommand,
    PauseCommand,
    PlayPlaylistCommand,
    PlayTrackCommand,
    ...
    ResumeCommand,
    SetVolumeCommand,
    StartConcentrationPhaseCommand,
    StartEveningPhaseCommand,
    SwitchDeviceCommand,
)
...
class SpotifyController:
    ...
    def play_playlist(self, playlist_identifier):
        return self.execute_command(
            PlayPlaylistCommand(self.player, playlist_identifier)
        )
    ...
    def start_concentration_phase(self, playlist_type=None):
        return self.execute_command(
            StartConcentrationPhaseCommand(self.player, playlist_type)
        )

    def start_evening_phase(self, playlist_type=None):
        return self.execute_command(
            StartEveningPhaseCommand(self.player, playlist_type)
        )
    ...
    def switch_device(self, device_name):
        return self.execute_command(
            SwitchDeviceCommand(self.device_manager, device_name)
        )
