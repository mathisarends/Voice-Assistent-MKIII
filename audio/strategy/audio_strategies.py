...
class SonosAudioStrategy(AudioPlaybackStrategy, LoggingMixin):
    ...
    def initialize(self):
        ...
        try:
            ...
            device = devices[0]
            self.logger.info("✅ Sonos-Lautsprecher automatisch gewählt: %s (%s)", 
    ...
    def _connect_by_ip(self, ip):
        ...
        try:
            ...
            self.logger.info("✅ Sonos-Lautsprecher gefunden: %s (%s)", device.player_name, ip)
            self.sonos_device = device
            self._initial_sonos_volume = self.sonos_device.volume
            return True
        except Exception as e:
            self.logger.error("❌ Konnte nicht mit Sonos-Lautsprecher unter %s verbinden: %s", ip, e)
            return False
        ...
        try:
            ...
            for device in devices:
                if device.player_name.lower() == name.lower():
                    self.logger.info("✅ Sonos-Lautsprecher gefunden: %s (%s)", 
                                    device.player_name, device.ip_address)
                    self.sonos_device = device
                    self._initial_sonos_volume = self.sonos_device.volume
                    return True
            
            self.logger.error("❌ Kein Sonos-Lautsprecher mit Namen '%s' gefunden", name)
            return False
        except Exception as e:
            self.logger.error("❌ Fehler bei der Suche nach Sonos-Lautsprecher '%s': %s", name, e)
            return False
    ...
    def _start_http_server(self):
        ...
        try:
            ...
            self.logger.info("✅ HTTP-Server für Sonos gestartet auf Port %d", self.http_server_port)
            return True
            ...
            self.logger.error("❌ Fehler beim Starten des HTTP-Servers für Sonos: %s", e)
            self.logger.warning("   Sonos-Funktionalität wird eingeschränkt sein!")
            return False
    ...
    def _check_url_availability(self, url):
        ...
        try:
            ...
            self.logger.warning("⚠️ Warnung: URL nicht erreichbar (Status %d): %s", 
                               response.status_code, url)
            self.logger.warning("   Stelle sicher, dass der HTTP-Server läuft und das Projektverzeichnis korrekt eingestellt ist.")
            return False
        except Exception as e:
