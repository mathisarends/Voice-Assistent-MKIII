...
class SecondBrainWorkflow(BaseGraph):
    ...
    async def _create_note(self, state: SecondBrainWorkflowState) -> SecondBrainWorkflowState:
        ...
        note_content = self._format_research_results_for_note(state["research_results"], state["search_query"])
    ...
    def _format_research_results_for_note(self, research_results, search_query: str) -> str:
        ...
        note_content += "Dies ist eine automatisch generierte Notiz mit Recherche-Ergebnissen.\n\n"
    ...
    async def _finalize_response(self, state: SecondBrainWorkflowState) -> SecondBrainWorkflowState:
        ...
        if isinstance(research_results, list):
            for idx, result in enumerate(research_results[:3], 1):  # Zeige nur die ersten 3 Ergebnisse
                if isinstance(result, dict):
                    title = result.get("title", f"Ergebnis {idx}")
                    url = result.get("url", "Keine URL verfügbar")
